...
int main()
{
    ...
    continue;
    ofstream out;
    out.open("/home/srg/Desktop/history.txt",ios::out| ios::ate| ios::app);
    out << command << endl; 
    out.close(); 
    if(command.find("record start") != std::string::npos)
    {log1=1;
      continue;}
      if(command.find("record stop") != std::string::npos)
    {log1=0;
      continue;}
    if(log1==1)
    {
    ...
    out.open("/home/srg/Desktop/log.txt",ios::out| ios::ate| ios::app);
    out << input;
    out << command << endl; 
    out<<output<<endl;
    out.close();
	...
	if (command.find("<") != std::string::npos)
      {
        ...
        if(strcmp(argv[0],"History")==0)
        {
          string line;
          ifstream in;
          in.open("/home/srg/Desktop/history.txt"); 
          while (in) { 
          getline(in, line); 
          cout << line<< endl;} 
          in.close(); 
    ...
    if(log1==1)
        {
          int ss;
          char *lv[64];
          string l1="/home/srg/Desktop/log.txt";
          char *lo1 = new char[l1.length()+1];
          strcpy(lo1,l1.c_str());
          ...
          if (pp == 0)
          {
              if(ofile!=NULL)
            {
                  int fd22=open(ofile, O_WRONLY| O_APPEND | O_CREAT, 0644);
                  dup2(fd22, STDOUT_FILENO);
                  close(fd22);
            }
          execvp(*argv,argv);
          }
